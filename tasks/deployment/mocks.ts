...
task('deploy:mock:ERC20', 'Deploy an ERC20 contract')
  ...
  .addParam('tokenDecimals', 'decimal points', TOKEN_DECIMALS, types.int)
  .addParam('tokens', 'amount of tokens', TOTAL_TOKENS, types.int)
  .setAction(async ({ name, symbol, tokenDecimals, tokens }, hre) => {
    ...
    const deployment = await hre.deployments.deploy('ERC20Mock', {
      ...
      args: [name, symbol, tokenDecimals, parseEther(tokens.toString())],
      log: true,
    });
